...
Implements global AGENTS.md section 19 `<security>` for this repo. Security items are part of the Definition of Done for every change, not a later phase.
...
The task-completion self-audit (global AGENTS.md section 15) now includes a **Security check** for this repo: local SAST clean (commands above); every touched input boundary names its injection class(es) and defense in the table above; the table is updated if a boundary was added or changed.
